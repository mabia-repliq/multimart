...
const productData = [
  ...
  },
  {
    ...
    status: "Sold",
    price: "$79",
    img: "https://img.freepik.com/premium-photo/men-fashion-leather-brown-shoes-isolated-white_107612-404.jpg?size=626&ext=jpg&ga=GA1.1.1268318720.1687747546&semt=sph",
    category: "Shoe",
  },
];
...
const ProductStatus = () => {
  ...
  return (
    <div className="mt-20 text-center">
      ...
      {/* tabs section */}
      <div className="mt-12 mx-24 flex flex-wrap">
        {statusNames?.map(statusName => (
          <p
            className={`text-lg text-teal-700 cursor-pointer mb-2 px-4 py-1 font-bold
             ${
      ...
      {/* table section */}
      <div className="sm:hidden md:hidden lg:inline-block xl:inline-block mt-8">
        <table class="table-auto">
          <thead className="bg-gray-100">
            <tr className="text-left">
              <th className="py-3 px-20 text-xl font-semibold text-teal-700">
                Image
              </th>
              <th className="py-3 px-20 text-xl font-semibold  text-teal-700">
                Name
            ...
            {statuses?.map(product => (
              <tr className="bg-white border-b">
                <td className="px-20 py-3">
                  <img className="h-12 w-16" src={product.img} />
                </td>
                <td className="px-20 py-3 text-gray-600">
                  <p>{product.name}</p>
                </td>
                <td className="px-20 py-3 text-gray-600">
                  <p>{product.price}</p>
                </td>
                <td className="px-20 py-3 font-semibold text-gray-700">
                  <p>{product.status}</p>
                </td>
                <td className="px-20 py-3 text-gray-600">
      ...
      {/* <div className="sm:inline-block md:inline-block lg:hidden xl:hidden 2xl:hidden mt-4 mb-12 grid w-auto sm:grid-cols-1 md:grid-cols-2 gap-5 ">
        {statuses?.map(product => (
          <div
            key={product.id}
      ...
      <div className="mt-8 sm:inline-block md:inline-block lg:hidden xl:hidden 2xl:hidden">
        <div className="md:mx-8 grid md:grid-cols-2 sm:grid-cols-1 gap-4">
          {statuses?.map(product => (
            <div className="bg-white h-full w-full border hover:drop-shadow-xl rounded-lg lg:rounded-b-none lg:rounded-r p-4 leading-normal gap-4">
              ...
              <div className="flex flex-col gap-1 text-left text-sm">
                ...
                <p className="mb-2 italic text-base text-gray-600">
                  {product.status}
                </p>
              </div>
              <div className="flex justify-between">
                ...
                <button>
                  <svg
                    ...
                    strokeWidth={1.5}
                    stroke="currentColor"
                    className="text-teal-800 hover:text-teal-900 w-6 h-6 transition duration-300 ease-in-out transform hover:-translate-y-1 hover:scale-110"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
